...
struct HeliParams {
  raylib::Vector3 init_pos;
  raylib::Vector3 init_rotation;

  double mass;              //  [Kg]
  raylib::Vector3 drag;     // [N / (M / SEC)]
  ...
  raylib::Vector3
      body_moment_of_inertia; //  Moment of inertia [Kg * M] coefficients
  ...
  std::vector<raylib::Vector3> touchpoints_in_heli;
...
public:
  ...
  virtual raylib::Vector3 get_position() const { return m_pos; }
  virtual void set_position(const raylib::Vector3 new_pos) { m_pos = new_pos; }
  virtual raylib::Vector3 get_velocity() const { return m_v; }
  virtual void set_velocity(const raylib::Vector3 new_v) { m_v = new_v; }
  virtual raylib::Vector3 get_gyro_angularv() const {
  ...
  virtual void reset_force() {
    m_external_force = raylib::Vector3();
    m_external_torque = raylib::Vector3();
  }
...
protected:
  ...
  raylib::Vector3 m_external_torque;
  raylib::Vector3 m_external_force;

  // Torbulat related.
  raylib::Vector3 torbulant_force(const raylib::Vector3 &pos_in_world,
  ...
  // Body-rotor forces.
  raylib::Vector3 calc_body_rotor_reaction_moment(float time_delta);
  raylib::Vector3 m_prev_reaction_in_body;

  // Tail force and torque.
  void calc_tail_rotor_force(const raylib::Vector3 &wind_speed,
                             raylib::Vector3 &out_force_in_world,
                             raylib::Vector3 &out_torque_in_world);

  // Engine torque.
  raylib::Vector3 calc_engine_torque();

  // The swash torque.
  raylib::Vector3 calc_swash_torque();

  // The aerodynamic forces and torques.
  void calc_aerodynamic_drag(const raylib::Vector3 &wind_speed,
                             raylib::Vector3 &out_force_in_world,
                             raylib::Vector3 &out_torque_in_world);

  // The lift forces and engine torques.
  void calc_lift_force(float time_delta, const raylib::Vector3 &wind_speed,
                       raylib::Vector3 &out_force_in_world,
                       raylib::Vector3 &out_rotor_torqu_in_world);

  // The dissimetry of lift.
  raylib::Vector3 calc_dissimetry_of_lift(const raylib::Vector3 &wind_speed,
                                          float lift_magnitude);

  // Body and rotor orientation properties.
  raylib::Vector3 m_rotor_angular_momentum_in_world;
  irr::core::matrix4 m_rotor_rotation;
  irr::core::matrix4 m_body_rotation;
  raylib::Vector3 m_body_angularv_in_body_coords;
